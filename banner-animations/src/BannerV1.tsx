...
/* ────────────────────────────────────────────────────────────────────────────
 * V1 — Multi-scene banner. The same card stays on screen across four service
 * pillars; the inner content (title, tabs, steps) cross-fades between scenes.
 *   1. Proposal Writing
...
// Inner-area / row coordinates (card-local).
...
/* ── Scene timing ──────────────────────────────────────────────────────── */
const SCENE_DUR = 180; // 6s per scene
const CROSS = 16; // cross-fade overlap (~0.5s)
const SCENE_STRIDE = SCENE_DUR - CROSS; // 164
export const TOTAL_FRAMES = SCENES.length * SCENE_STRIDE + CROSS; // 4*164 + 16 = 672
...
/* ── One full scene's content (header + tabs + 5 step rows) ────────────── */

type ScenarioContentProps = {
  scene: Scene;
  /** Local frame within this scene (0 → SCENE_DUR). */
  localFrame: number;
};

const ScenarioContent: React.FC<ScenarioContentProps> = ({ scene, localFrame }) => {
  // Header / tabs entry inside the scene local timeline.
  ...
  // Sequential progress fills.
  const seqStart = 30;
  const fillDur = 20;
  const checkDur = 6;
  const gap = 2;
  const stepBlock = fillDur + checkDur + gap; // 28; 5 blocks = 140 → ends at 170

  const fillFor = (i: number) =>
    interpolate(localFrame, [seqStart + i * stepBlock, seqStart + i * stepBlock + fillDur], [0, 1], {
  ...
  // Active tab (uses scene-level boundaries).
  let activeTab = 0;
  ...
  return (
    <BannerStage cardEntryStart={6} cardEntryDuration={24}>
      {SCENES.map((scene, i) => {
        ...
        // Scene visibility:
        // 0 → 1 over first CROSS frames (cross-fade in)
        // 1 hold
        // 1 → 0 over last CROSS frames (cross-fade out, except last scene which holds)
        ...
        // Subtle cross-fade lift (incoming rises 6→0, outgoing sinks 0→-6).
